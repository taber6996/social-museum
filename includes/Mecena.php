...
class Mecena
{
    ...
    public static function buscaMecena($id_usuario, $id_artista)
    {
        ...
        $conn = $app->conexionBd();
		$query = sprintf("SELECT * FROM Mecenas M WHERE M.id_usuario = %d AND M.id_artista = %d",$id_usuario,$id_autor);
        $rs = $conn->query($query);
        $result = false;
        if ($rs) {
            if ( $rs->num_rows == 1) {
                $fila = $rs->fetch_assoc();$mecena = new Mecena($fila['id_usuario'], $fila['id_artista']);
